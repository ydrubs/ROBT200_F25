...
from rclpy.parameter import Parameter  # Import to allow for parameter callback


class NumberPublisherNode(Node):
    def __init__(self):
        super().__init__("number_publisher_w_params")  # name of the node

        # Below we create parameters for the starting number and the update frequency
        # We then need to update out publisher with the associated parameters
        self.declare_parameter("number", 2)
        self.declare_parameter("publish_period", 1.0)

        # Register a parameter callback
        # Name of callback is the only argument needed
        # This allows parameter to be changed during run-time
        self.add_post_set_parameters_callback(self.parameters_callback)

        # Add an attribute to keep count
        # self.number = 2

        # Instead of setting a static number like in the line above, we can now grab the value of the parameter.
        # We do the same for the frequency
